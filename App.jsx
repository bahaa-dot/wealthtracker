...
export default function WealthTracker() {
  ...
  useEffect(() => {
    const savedAuth = localStorage.getItem('wealthTrackerAuth');
    if (savedAuth) {
      const authData = JSON.parse(savedAuth);
      setIsAuthenticated(true);
      setCurrentUser(authData.username);
      loadUserAssets(authData.username);
    } else {
      const sampleAssets = [
        {
          id: 1,
  ...
  const loadUserAssets = (username) => {
    const userAssets = localStorage.getItem(`assets_${username}`);
    if (userAssets) {
      setAssets(JSON.parse(userAssets));
    }
  };

  const saveUserAssets = (username, assetsData) => {
    localStorage.setItem(`assets_${username}`, JSON.stringify(assetsData));
  };
  ...
  const handleLogin = () => {
    ...
    if (loginForm.username === FIXED_USERNAME && loginForm.password === FIXED_PASSWORD) {
      const authData = { username: FIXED_USERNAME };
      localStorage.setItem('wealthTrackerAuth', JSON.stringify(authData));
      setIsAuthenticated(true);
      setCurrentUser(FIXED_USERNAME);
      loadUserAssets(FIXED_USERNAME);
  ...
  const handleLogout = () => {
    localStorage.removeItem('wealthTrackerAuth');
  ...
  const addAsset = () => {
    ...
    const { gainAmount, gainPercent } = calculateGains(
      formData.units,
    ...
    if (currentUser) {
      saveUserAssets(currentUser, updatedAssets);
    }
  ...
  const exportData = () => {
    ...
    link.href = url;
    link.download = `portfolio_${currentUser || 'preview'}_${new Date().toISOString().split('T')[0]}.json`;
    link.click();
  };
      ...
      return (
        <div className="bg-slate-800 border border-slate-600 rounded-lg p-3 shadow-lg">
          ...
          {payload[0].payload.percentage && (
            <p className="text-slate-400 text-sm">{payload[0].payload.percentage}%</p>
          )}
        </div>
      );
    }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 bg-slate-800 px-4 py-2 rounded-lg border border-slate-700">
              ...
              <span className="text-white font-medium">{currentUser || 'Preview Mode'}</span>
            </div>
            <button
              onClick={handleLogout}
              className="flex items-center gap-2 bg-slate-700 hover:bg-slate-600 text-white px-4 py-2 rounded-lg font-medium transition"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
            ...
            <p className={`text-3xl font-bold ${totalGains >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              ${Math.abs(totalGains).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </p>
            <p className={`text-sm ${totalGains >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              {totalGains >= 0 ? '+' : '-'}{Math.abs(totalGainPercent).toFixed(2)}%
            </p>
          </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {Object.entries(getTotalsByType()).map(([type, data]) => (
                      <div key={type} className="bg-slate-900 rounded-lg p-4 border border-slate-600">
                        ...
                        <p className={`text-sm ${data.gain >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                          {data.gain >= 0 ? '+' : ''}{data.gain.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </p>
                      </div>
                    ))}
                  </div>
                    ...
                    <div className="space-y-3">
                      {getWorstPerformers().map((asset) => (
                        <div key={asset.id} className="flex items-center justify-between bg-slate-900 rounded-lg p-3 border border-slate-600">
                          ...
                          <div className="text-right">
                            <p className={`font-medium ${asset.gainPercent >= 0 ? 'text-green-400' : 'text-red-400'}`}>
